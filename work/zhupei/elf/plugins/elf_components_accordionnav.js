...
(function(){
    ...
    function openCurrent(target,currentId){
        var currentItem=target.nodeList[currentId];
        if(currentItem){
            setCurrent(target,currentItem);
            openParents(target,currentItem);
        }
    }
    function setCurrent(target,currentItem){
        var opts=target.options;
        //var currentItem=target.nodeList[currentId];
        if(Elf.utils.hasClass(currentItem,"fold")){
            toggleTreeNode(target,currentItem);
        }else{
            active(target,currentItem);
            if(opts.onClick && typeof opts.onClick=="function"){
                opts.onClick.call(target,currentItem);
            }
        }
    }
    ...
    Elf.components.accordionNav.methods = {
        ...
        openCurrent:function(target,currentId){
            return openCurrent(target,currentId);
        },
