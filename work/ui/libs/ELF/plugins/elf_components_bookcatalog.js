...
(function(){
    ...
    function active(target,item){
        ...
        if(opts.vie){
            var siblings=Elf.utils.siblings(item);
    ...
    function unSelected(target,node){
        ...
        /*if(node.childNodeLs && node.childNodeLs.children.length>0){
            Elf.utils.each(node.childNodeLs.children,function(index,el){
                unActive(target,el);
            });
        }*/
    }
    ...
    /*function openParents(target,item){
        if(item.parentKey>0){
            ...
            if(parent.parentKey && parent.parentKey!="0"){
    ...
    function creatNodeItem(target,obj){
        var opts=target.options;
        //var item=opts.createNode(target,obj);
        var item=opts.createNode(target,obj);
        item.ownData=obj;
        item.key=obj[opts.key];
        //item.parentKey=obj[opts.parentKey];
        //item.childrenList=[];
        //item.childNodeLs=Elf.createChild(item,{name: "ul"});
    ...
    function createNode(target,obj,cover,pNode){
        ...
        var link=Elf.controls.createElement("a","elf-accordion-point",{innerHTML:obj.name},accordionNode);
        node.ownData=obj;
    ...
    function createSections(target,obj){
        var node=Elf.controls.createElement("li",target.childNodeLs);
        var accordionNode=Elf.controls.createElement("div","elf-accordion-node",node);
        var handle=Elf.controls.createElement("i","elf-accordion-handle",accordionNode);
        var link=Elf.controls.createElement("a","elf-accordion-point",{innerHTML:obj.name},accordionNode);
        node.ownData=obj;
